...
# -*- coding: utf-8 -*-
"""
Enhanced CultureMoE 模型评估脚本（权重压缩版本）
...
# 导入权重压缩工具
from model_utils_small import load_moe_weights_smart, get_compression_info, find_best_weight_file
...
class EnhancedCultureMoEEvaluator:
    ...
    def load_enhanced_culturemoe_from_components(self) -> Tuple[EnhancedCultureMoE, AutoTokenizer]:
        ...
        # 智能查找最佳权重文件（支持压缩格式）
        best_moe_weights_file = find_best_weight_file(moe_weights_file)
        if best_moe_weights_file is None:
            raise FileNotFoundError(f"Enhanced MoE weights not found: {moe_weights_file}")

        # 获取压缩信息并记录
        compression_info = get_compression_info(best_moe_weights_file)
        logging.info(f"Loading MoE weights from: {best_moe_weights_file}")
        logging.info(f"Compression type: {compression_info.get('compression_type', 'none')}")
        logging.info(f"File size: {compression_info.get('file_size_mb', 0):.1f} MB")

        # 智能加载MoE权重（自动处理压缩格式）
        moe_state_dict = load_moe_weights_smart(best_moe_weights_file, target_device="cpu")
