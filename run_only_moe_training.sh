...
echo "======================================="
echo "只训练最后两层MoE"
echo "替换最后两层FFN为MoE专家（LoRA适配器）"
echo "======================================="

# 参数设置
BACKBONE=${1:-"qwen"}  # 默认使用qwen2.5-7B
DATA_ID=${2:-"2"}
NUM_MOE_EXPERTS=${3:-"4"}  # 每层MoE专家数量
NUM_GPUS=${4:-"2"}
LORA_RANK=${5:-"16"}   # LoRA rank
LORA_ALPHA=${6:-"32"}  # LoRA alpha

# 检查参数
if [ "$#" -gt 6 ]; then
    echo "❌ 参数过多！用法: $0 [backbone] [data_id] [num_moe_experts] [num_gpus] [lora_rank] [lora_alpha]"
    exit 1
fi
...
OUTPUT_DIR="/root/autodl-fs/only_moe/${MODEL_NAME}_${DATASET_TAG}_${TIMESTAMP}"

echo "配置信息:"
echo "  模型: $MODEL_NAME ($BASE_MODEL)"
echo "  数据: $DATASET_TAG ($TRAIN_FILE)"
echo "  总层数: $TOTAL_LAYERS"
echo "  训练模式: 只训练最后两层MoE"
echo "  MoE层: $MOE_LAYERS"
echo "  每层专家数: $NUM_MOE_EXPERTS"
...
cat > "$OUTPUT_DIR/config.json" << EOF
{
    ...
    "training_config": {
        ...
        "moe_experts_per_layer": $NUM_MOE_EXPERTS,
        "lora_rank": $LORA_RANK,
...
if [ $TRAINING_SUCCESS -eq 0 ]; then
    ...
    # 检查最佳模型
    BEST_MODEL="$OUTPUT_DIR/best_only_moe_model"
    if [ -d "$BEST_MODEL" ]; then
        ...
        echo "  - 每层专家数: $NUM_MOE_EXPERTS"
        echo "  - LoRA配置: rank=$LORA_RANK, alpha=$LORA_ALPHA"
        echo "  - 冻结参数: 除最后两层MoE外的所有参数"
        echo "  - 序列长度: $MAX_SEQ_LEN"
    else
        echo "⚠️  训练完成但未找到最佳模型"
    fi
else
    echo "❌ 只训练最后两层MoE 失败！退出码: $TRAINING_SUCCESS"
    echo "故障排查："
...
if [ $TRAINING_SUCCESS -eq 0 ]; then
    echo "  最佳模型: $OUTPUT_DIR/best_only_moe_model/"
fi
