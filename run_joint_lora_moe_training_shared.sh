...
# 联合训练脚本：同时训练预训练LoRA适配器 + 新增MoE处理层 + Shared专家
# 实现端到端的LoRA+MoE+Shared专家联合优化训练
# 针对48GB×2卡优化

echo "======================================="
echo "联合训练：LoRA + MoE + Shared专家 端到端优化"
echo "同时训练预训练LoRA适配器、新增MoE层和Shared专家"
echo "======================================="
...
case $DATA_ID in
    ...
    4)
        ...
        ;;
    *)
        echo "❌ 无效的DATA_ID: $DATA_ID (支持: 1, 2, 3, 4)"
        exit 1
        ;;
esac
...
echo "  训练模式: 联合训练 (LoRA + MoE + Shared专家)"
...
echo "  Shared专家学习率: $LEARNING_RATE_SHARED"
...
if [ "$NUM_GPUS" -eq 1 ]; then
    # 单卡训练
    python train_joint_lora_moe_shared.py \
    ...
    torchrun \
        ...
        --master_port=29500 \
        train_joint_lora_moe_shared.py \
        --base_model_path "$BASE_MODEL" \
        --train_file "$TRAIN_FILE" \
        --output_dir "$OUTPUT_DIR" \
        --data_id "$DATA_ID" \
...
if [ $TRAINING_SUCCESS -eq 0 ]; then
    echo "✅ 联合训练 LoRA + MoE + Shared专家 成功！"
    ...
    echo "❌ 联合训练 LoRA + MoE + Shared专家 失败！退出码: $TRAINING_SUCCESS"
    echo "故障排查："
