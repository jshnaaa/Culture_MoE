...
# -*- coding: utf-8 -*-
"""
增强的 CultureMoE 训练脚本 - 从检查点恢复训练版本
支持从之前保存的检查点继续训练
...
# ===== 训练器类 =====
class EnhancedCultureMoEResumeTrainer:
    """增强的 CultureMoE 训练器 - 从检查点恢复版本"""
    ...
    def setup_logging(self):
        """设置日志"""
        log_file = os.path.join(self.args.resume_dir, 'training_resumed.log')
        logging.basicConfig(
            level=logging.INFO,
        ...
        logging.info("=" * 80)
        logging.info("Enhanced CultureMoE Training Resumed")
        logging.info("=" * 80)
    ...
    def load_original_config(self):
        ...
        if os.path.exists(config_file):
            ...
            logging.warning("Original config.json not found, using current arguments")
    ...
    def load_model(self):
        ...
        # 创建Enhanced CultureMoE模型
        logging.info("Creating Enhanced CultureMoE model...")
        self.model = EnhancedCultureMoE(
            llama_model=merged_model,
            config=merged_model.config,
            args=model_args,
            culture_loss_lambda=getattr(self.args, 'culture_loss_lambda', 0.5),
            moe_fusion=getattr(self.args, 'moe_fusion', 0.4),
            num_cultures=6,
            culture_dim=256
        )
    ...
    def setup_optimizer(self):
        ...
        learning_rate = getattr(self.args, 'learning_rate', 2e-4)
        moe_lr_multiplier = getattr(self.args, 'moe_lr_multiplier', 1.0)
        ...
        eval_batch_size = getattr(self.args, 'eval_batch_size', 1)
    ...
    def train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            outputs = self.model(
                ...
                culture_loss_lambda=getattr(self.args, 'culture_loss_lambda', 0.5),
                culture_loss_alpha=getattr(self.args, 'culture_loss_alpha', 2.0),
                culture_loss_beta=getattr(self.args, 'culture_loss_beta', 1.0),
                use_shared_experts=getattr(self.args, 'use_shared_experts', True),
                router_temperature=getattr(self.args, 'router_temperature', 2.0),
                load_balance_weight=getattr(self.args, 'load_balance_weight', 0.01),
                entropy_weight=getattr(self.args, 'entropy_weight', 0.1)
            )
    ...
    def save_model(self, epoch: int, is_best: bool = False):
        ...
        if is_best:
            ...
            logging.info(f"Saving new best model to {save_dir}")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="Enhanced CultureMoE Resume Training")
    ...
    print("\n" + "=" * 80)
    print("Enhanced CultureMoE Resume Training")
    print("=" * 80)
    ...
    # 创建训练器并开始训练
    trainer = EnhancedCultureMoEResumeTrainer(args)
    results = trainer.train()

    print("\n" + "=" * 80)
    print("✅ Enhanced CultureMoE Resume Training Completed!")
    print("=" * 80)
