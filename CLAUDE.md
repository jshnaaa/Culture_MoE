# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## 项目概述

Culture_Moe 是一个基于 Mixture of Experts (MoE) 架构的文化感知语言模型研究项目。该项目实现了 CultureMoE 模型，通过专家路由机制和文化损失函数来提升模型对不同文化背景的理解和生成能力。

## 核心架构组件

### 1. CultureMoE 模型架构
- **纯MoE架构**: 所有层的FFN都替换为LoRA MoE结构
- **文化感知路由**: 通过文化损失引导专家学习文化特定模式
- **内存高效**: 基于LoRA的专家设计，显著降低参数量和内存需求
- **主要组件**:
  - LoRA专家 (`LoRAExpert`): 参数高效的专家网络
  - MoE路由器 (`MoERouter`): 动态专家选择和权重分配
  - MoE FFN层 (`MoEFFNLoRA`): 完整的专家混合前馈网络
  - 简化版适配器 (`SimplifiedCultureMoEAdapter`): 模型改造和训练管理

### 2. 损失函数体系
- **总损失**: `L_total = L_generation + λ × (α × L_balance + β × L_culture)`
- **生成损失**: 标准自回归语言建模损失
- **负载均衡损失**: 防止专家使用不均衡
- **文化对比损失**: 基于余弦相似度的对比学习，促进专家文化专业化

### 3. 支持的基础模型
- LLaMA 3.1-8B-Instruct (默认)
- Qwen 2.5-7B-Instruct

## 常用开发命令

### 环境设置
```bash
# 安装依赖
pip install -r requirements.txt

# 代码质量检查
make quality

# 代码格式化
make style

# 运行测试
make test

# 提交前检查
make commit
```

### 训练命令

#### 1. LoRA Only 训练
```bash
# 训练+验证
sh run_train_lora_only.sh llama 3 true

# 生成式训练
sh run_train_lora_only_gen.sh llama 4

# 生成式测试
sh run_eval_lora_only_from_components.sh llama
```

#### 2. CultureMoE 训练

**简化版训练（推荐）**:
```bash
# 基础训练
./run_simplified_culturemoe.sh llama 2 true true 4 new 2 true 2 16 32

# 参数说明：
# backbone data_id use_shared use_gate num_experts culture_loss activated_experts
# lambda alpha beta use_lora num_gpus lora_rank lora_alpha
```

**端到端训练**:
```bash
sh run_train_ddp_lora_dual.sh llama 3 True false
```

**两阶段训练**:
```bash
# 1. 先完成LoRA only训练
# 2. 合并LoRA
sh run_merge_lora.sh llama 2

# 3. 训练MoE
sh run_train_culturemoe_from_merged.sh llama 2 True 6 false 1 true true
```

#### 3. 基础模型评估
```bash
# LLaMA + 4分类
sh run_eval_base_llama.sh llama 41

# 生成式评估
sh run_eval_base_gen.sh llama 11
```

### 消融实验
```bash
# 运行完整消融实验
sh run_ablation_study.sh

# VSM13 文化一致性测试
sh run_eval_vsm13.sh base llama
sh run_eval_vsm13.sh lora qwen 2
sh run_eval_vsm13.sh moe llama 4
```

## 项目结构

```
Culture_Moe/
├── src/llamafactory/          # 核心模型实现
│   └── model/
│       ├── CultureMoE.py      # 主要MoE模型类
│       ├── simplified_culturemoe.py        # 简化版配置
│       └── simplified_culturemoe_adapter.py # 适配器实现
├── train_*.py                 # 各种训练脚本
├── eval_*.py                  # 各种评估脚本
├── run_*.sh                   # 训练/评估执行脚本
├── ft_*.py                    # 微调相关脚本
├── requirements.txt           # Python依赖
├── culturemoe.md             # 详细架构文档
├── readme.md                 # 使用指南
└── examples/                 # 示例和教程
```

## 配置参数

### 模型参数
- `num_moe_experts`: MoE专家数量 (默认: 4)
- `num_activated_experts`: 激活的专家数量 (默认: 2)
- `lora_rank`: LoRA rank (默认: 16)
- `lora_alpha`: LoRA alpha (默认: 32)
- `use_shared`: 是否使用共享专家 (默认: false)
- `use_gate`: 是否使用门控网络 (默认: false)

### 训练参数
- `learning_rate`: 学习率 (默认: 1e-4)
- `batch_size`: 批次大小 (默认: 4)
- `gradient_accumulation`: 梯度累积步数 (默认: 8)
- `num_epochs`: 训练轮数 (默认: 7)
- `max_seq_len`: 最大序列长度 (动态设置: 256/512/769)

### 损失函数权重
- `lambda`: 辅助损失总权重 (默认: 1.0)
- `alpha`: 负载均衡损失权重 (默认: 0.1)
- `beta`: 文化对比损失权重 (默认: 0.5)

## 数据集配置

支持的数据集 (通过 DATA_ID 参数指定):
- `1`: blend_merge_gen.json
- `2`: CulturalBench_merge_gen.json
- `3`: normad_merge_gen.json
- `4`: cultureLLM_merge_gen.json
- `5`: cultureAtlas_merge_gen.json
- `15`: blend + cultureAtlas 合并数据集

## 内存优化策略

### GPU内存配置
- **单卡训练**: 7-9GB显存
- **双卡训练**: 4-5GB per GPU
- **推理阶段**: 6-8GB显存

### 环境变量优化
```bash
export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:64,garbage_collection_threshold:0.6
export CUDA_LAUNCH_BLOCKING=0
export TOKENIZERS_PARALLELISM=false
export OMP_NUM_THREADS=2
```

### 分布式训练配置
- 启用 `find_unused_parameters` 处理top-k路由的动态计算图
- 禁用 `broadcast_buffers` 减少同步开销
- 启用 `gradient_as_bucket_view` 优化梯度存储

## 代码质量标准

### Linting 配置 (ruff)
- 目标Python版本: 3.9+
- 行长度限制: 119字符
- 缩进宽度: 4空格
- 引用风格: 双引号

### Pre-commit Hooks
- AST语法检查
- 大文件检查 (最大25MB)
- 合并冲突检查
- YAML格式检查
- 调试语句检查
- 文件末尾换行符
- 行尾空格清理

## 关键文件位置

### 训练脚本
- `train_simplified_culturemoe.py`: 简化版CultureMoE训练
- `train_joint_lora_moe.py`: 联合LoRA+MoE训练
- `ft_lora_only_gen.py`: LoRA生成式微调

### 评估脚本
- `eval_simplified_culturemoe.py`: 简化版模型评估
- `eval_culturemoe_from_components.py`: 组件式模型评估
- `eval_base_gen.py`: 基础模型生成式评估

### 核心模型实现
- `src/llamafactory/model/simplified_culturemoe_adapter.py`: 简化版适配器
- `src/llamafactory/model/CultureMoE.py`: 主要MoE模型类

## 故障排查

### 常见问题
1. **显存不足**:
   - 减少batch_size
   - 增加gradient_accumulation_steps
   - 降低max_seq_len

2. **训练不稳定**:
   - 检查learning_rate设置
   - 验证损失函数权重配置
   - 确认数据预处理正确性

3. **分布式训练问题**:
   - 确认CUDA_VISIBLE_DEVICES设置
   - 检查master_port是否被占用
   - 验证nproc_per_node参数

### 日志位置
- 训练日志: `{OUTPUT_DIR}/training.log`
- 配置文件: `{OUTPUT_DIR}/config.json`
- 最佳模型: `{OUTPUT_DIR}/best_simplified_culturemoe/`

## 开发最佳实践

1. **模型开发**: 优先使用简化版架构，在所有层替换FFN为MoE结构
2. **实验管理**: 使用消融实验脚本系统性评估不同配置
3. **内存优化**: 针对48GB×2卡环境进行参数调优
4. **文化损失**: 重点关注文化对比损失的数值稳定性
5. **代码质量**: 提交前运行 `make commit` 确保代码质量