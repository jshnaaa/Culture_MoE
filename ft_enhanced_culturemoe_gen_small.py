...
# 导入权重压缩工具
from model_utils_small import save_moe_weights_compressed, get_compression_info
...
class EnhancedCultureMoETrainer:
    ...
    def save_model(self, epoch: int, is_best: bool = False):
        ...
        # 保存MoE权重 (使用压缩格式)
        base_moe_weights_path = os.path.join(save_dir, 'moe_weights.pth')
        ...
        # 获取压缩信息
        compression_info = get_compression_info(moe_weights_path)

        logging.info(f"MoE weights saved to {save_dir}")
        logging.info(f"  Parameters: {moe_param_count:,}")
        logging.info(f"  Compressed file size: {file_size_mb:.1f} MB")
        logging.info(f"  Expected uncompressed size: {param_size_mb:.1f} MB")
        logging.info(f"  Compression type: {compression_info.get('compression_type', 'unknown')}")
        logging.info(f"  Space saved: ~{100 * (1 - file_size_mb / param_size_mb):.1f}%")

        if file_size_mb > 1000:  # 如果超过1GB，发出警告
