...
# ============================================================
# 使用修复版本的 MiLoRA (Matrix-informed Low-Rank Adaptation) 微调模型
#
# 修复内容：
# 1. 调整初始化验证阈值从1e-3到5e-3，避免误报
# 2. 改进SVD分解的数值稳定性，使用双精度计算
# 3. 增强错误处理和异常恢复机制
# 4. 添加更详细的诊断信息和日志记录
# 5. 优化数据类型转换过程，减少精度损失
#
...
#
# 使用方法：
#   sh run_ft_milora_fixed.sh <BACKBONE> <DATA_ID>
#
# 参数说明：
...
#   sh run_ft_milora_fixed.sh llama 2
#
#   # 使用 Qwen + CultureLLM 数据集
#   sh run_ft_milora_fixed.sh qwen 4
# ============================================================
...
# 输出目录
OUTPUT_DIR="/root/autodl-fs/data/ft/ft_milora_fixed_gen_${DATASET_TAG}_${BACKBONE}_$(date +%Y%m%d_%H%M)"

echo "============================================================"
echo "Fine-tuning with Fixed MiLoRA (Matrix-informed Low-Rank Adaptation)"
echo "============================================================"
echo "Method: Fixed MiLoRA - SVD-based initialization with improved stability"
...
echo "  ✓ Enhanced SVD numerical stability (double precision)"
...
echo ""
echo "Fixed MiLoRA Key Features:"
...
echo "  Training method: Fixed MiLoRA (rank=64, dropout=0.05)"
...
# 运行训练
echo "Starting Fixed MiLoRA training..."
echo ""
...
if [ $? -eq 0 ]; then
    echo ""
    echo "============================================================"
    echo "✅ Fixed MiLoRA Training completed successfully!"
    echo "============================================================"
    echo "Results saved to: $OUTPUT_DIR"
    echo ""
    echo "🎯 Initialization Issues Resolved:"
    echo "  ✓ No more false verification failures"
    echo "  ✓ Improved SVD numerical stability"
    ...
    echo ""
    echo "Fixed MiLoRA Model Structure:"
    ...
    echo ""
    echo "🔬 Fixed MiLoRA vs Standard LoRA Comparison:"
    echo "   - Initialization: SVD-based (Fixed MiLoRA) vs Random (LoRA)"
    echo "   - Verification: Robust threshold vs None"
    echo "   - Numerical stability: Enhanced vs Standard"
    ...
    echo ""
    echo "============================================================"
    echo "❌ Fixed MiLoRA Training failed!"
    echo "============================================================"
