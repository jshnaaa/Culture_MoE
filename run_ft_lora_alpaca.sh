...
case $DATA_ID in
    2)
        ...
        TRAIN_FILE="/root/autodl-fs/CulturalBench_merge_gen_alpaca.json"
        DATASET_TAG="CulturalBench"
        echo "Using CulturalBench dataset (Alpaca format)"
        ;;
    3)
        # NormAD
        DATASET_NAME="NormAD"
        TRAIN_FILE="/root/autodl-fs/normad_merge_gen_alpaca.json"
        DATASET_TAG="normad"
        echo "Using NormAD dataset (Alpaca format)"
        ;;
    4)
        # CultureLLM (默认)
        DATASET_NAME="CultureLLM"
        TRAIN_FILE="/root/autodl-fs/cultureLLM_merge_gen_alpaca.json"
        DATASET_TAG="cultureLLM"
        echo "Using CultureLLM dataset (Alpaca format)"
        ;;
    *)
...
# 输出目录
OUTPUT_DIR="/root/autodl-tmp/CultureMoE/Culture_Alignment/ft/ft_lora_alpaca_${DATASET_TAG}_${BACKBONE}_$(date +%Y%m%d_%H%M)"

echo "============================================================"
echo "Fine-tuning LoRA Only Model with Alpaca Format"
echo "============================================================"
...
if [ ! -f "$TRAIN_FILE" ]; then
    ...
    echo ""
    echo "Please first convert the data to Alpaca format:"
    echo "  python convert_culturellm_to_alpaca.py \\"
    echo "    --input_file /root/autodl-fs/${DATASET_TAG}_merge_gen.json \\"
    echo "    --output_file $TRAIN_FILE"
    exit 1
fi
