...
# ============================================================
# 🚀 ENHANCED CULTUREMOE TRAINING SCRIPT (权重压缩版本)
# 使用增强的文化感知组件训练 CultureMoE 模型
# 支持可配置的专家数量，便于消融实验
# 支持权重压缩保存，减少50%存储空间
# ============================================================
# 增强的 CultureMoE 训练脚本 (权重压缩版本)
#
# ✅ 新增功能：
...
#   5. 增强的文化损失：多组件文化信息综合
#   6. 可配置专家数量：支持消融实验
#   7. 权重压缩保存：FP16压缩，减少50%存储空间
#
# 使用方法：
#   sh run_ft_enhanced_culturemoe_gen_small.sh <BACKBONE> <DATA_ID> <USE_CULTURE_LOSS> <NUM_EXPERTS> <MOE_FUSION> <CULTURE_LOSS_WEIGHT> <MARGIN> <LAMBDA_DIFF> <USE_SHARED> <ROUTER_TEMP> <LOAD_BAL> <ENTROPY> <NUM_GPUS> <USE_MASK>
#
# 参数说明：
...
# 示例：
#   # 单GPU训练（默认）- 权重压缩版本
#   sh run_ft_enhanced_culturemoe_gen_small.sh llama 4 True 12 0.4 0.5 0.5 1.0 True 2.0 0.001 0.01 1
#
#   # 双GPU训练 - 权重压缩版本
#   sh run_ft_enhanced_culturemoe_gen_small.sh llama 4 True 12 0.4 0.5 0.5 1.0 True 2.0 0.001 0.01 2
#
#   # 消融实验：使用6个专家（单GPU）- 权重压缩版本
#   sh run_ft_enhanced_culturemoe_gen_small.sh llama 4 True 6 0.4 0.5 0.5 1.0 True 2.0 0.001 0.01 1
#
# ✨ 权重压缩优势：
#   - 文件大小减少约50% (2GB → 1GB)
#   - 加载速度基本不变
...
OUTPUT_DIR="/root/autodl-fs/data/ft/ft_enhanced_moe_gen_small_${DATASET_TAG}_${BACKBONE}_experts${NUM_EXPERTS}_${SHARED_TAG}_${MASK_TAG}_fusion${MOE_FUSION}_lambda${LAMBDA}_$(date +%Y%m%d_%H%M)"
...
echo "============================================================"
echo "🚀 Enhanced CultureMoE Training with Cultural Awareness Components (权重压缩版本)"
echo "============================================================"
...
echo "Compression: FP16 权重压缩，减少约50%存储空间"
echo ""
...
echo "  - Weight compression: FP16 format reduces file size by ~50%"
echo ""
...
echo "  - Compression type: FP16"
echo "  - Space saving: ~50% (2GB → 1GB)"
echo "  - Loading compatibility: Automatic format detection"
echo "  - Performance impact: Minimal (加载速度基本不变)"
echo ""
...
if [ ! -d "$LORA_WEIGHTS_PATH" ]; then
    ...
    echo "  sh run_ft_lora_only_gen.sh $BACKBONE"
    exit 1
fi

echo "Found LoRA weights: $LORA_WEIGHTS_PATH"
echo ""
...
# 运行训练 (使用压缩版本的脚本)
echo "Starting enhanced CultureMoE training with weight compression..."
echo ""

python ft_enhanced_culturemoe_gen_small.py \
...
if [ $? -eq 0 ]; then
    echo ""
    echo "============================================================"
    echo "✅ Enhanced CultureMoE Training (权重压缩版本) completed successfully!"
    echo "============================================================"
    echo "Results saved to: $OUTPUT_DIR"
    echo ""
    echo "Files generated:"
    echo "  - best_enhanced_moe/ (Best Enhanced MoE weights - 压缩格式)"
    ...
    echo ""
    echo "💾 Compression Benefits:"
    echo "  - MoE weights saved in FP16 format"
    echo "  - File size reduced by ~50%"
    echo "  - Compatible with both compressed and uncompressed evaluation scripts"
    echo ""
    ...
    echo "   sh run_ft_enhanced_culturemoe_gen_small.sh llama 4 True 12 0.4 0.5"
    echo ""
    echo "   # Dual GPU training:"
    echo "   sh run_ft_enhanced_culturemoe_gen_small.sh llama 4 True 12 0.4 0.5 0.5 1.0 True 2.0 0.001 0.01 2"
    echo ""
    echo "   # Evaluation with compressed weights:"
    echo "   sh run_eval_enhanced_culturemoe_from_components_small.sh"
    echo "============================================================"
else
    echo ""
    echo "============================================================"
    echo "❌ Enhanced CultureMoE Training (权重压缩版本) failed!"
    echo "============================================================"
    exit 1
fi