...
#
# 使用方法:
#   bash mad/run_eval_mad.sh MODEL_TYPE DATA_ID [MAX_SAMPLES] [RANDOM_P]
#
# 参数说明:
...
#   bash mad/run_eval_mad.sh 1 2
#
#   # 快速测试：Qwen在NORMAD上取前50个样本
#   bash mad/run_eval_mad.sh 2 3 50
#
#   # 随机采样：LLaMA在CultureLLM上随机取10%数据
#   bash mad/run_eval_mad.sh 1 4 0
#
#   # 随机采样：Qwen在CultureAtlas上随机取20%数据
#   bash mad/run_eval_mad.sh 2 5 0 0.2
#
#   # 完整评估：LLaMA在CulturalBench上使用全部数据
#   bash mad/run_eval_mad.sh 1 2 0 1.0
#
# ============================================================================
...
if [ "$MODEL_TYPE" -ne 1 ] && [ "$MODEL_TYPE" -ne 2 ]; then
    ...
    echo ""
    echo "使用方法:"
    echo "  bash mad/run_eval_mad.sh MODEL_TYPE DATA_ID [MAX_SAMPLES] [RANDOM_P]"
    echo ""
    echo "参数:"
...
OUTPUT_DIR="/root/autodl-fs/mad_results/${MODEL_NAME}_${DATA_NAME}_${TIMESTAMP}"
mkdir -p $OUTPUT_DIR
...
# 构建命令
CMD="python $SCRIPT_DIR/eval_mad.py \
