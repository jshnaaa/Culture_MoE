...
#
# 使用方法:
#   bash mad/run_eval_mad_5cultural.sh MODEL_TYPE DATA_ID [MAX_SAMPLES] [RANDOM_P]
#
# 参数说明:
...
#   bash mad/run_eval_mad_5cultural.sh 1 2
#
#   # 快速测试：Qwen在NORMAD上取前50个样本
#   bash mad/run_eval_mad_5cultural.sh 2 3 50
#
#   # 随机采样：LLaMA在CultureLLM上随机取10%数据
#   bash mad/run_eval_mad_5cultural.sh 1 4 0
#
#   # 随机采样：Qwen在CultureAtlas上随机取20%数据
#   bash mad/run_eval_mad_5cultural.sh 2 5 0 0.2
#
#   # 完整评估：LLaMA在CulturalBench上使用全部数据
#   bash mad/run_eval_mad_5cultural.sh 1 2 0 1.0
#
# ============================================================================
...
if [ "$MODEL_TYPE" -ne 1 ] && [ "$MODEL_TYPE" -ne 2 ]; then
    ...
    echo ""
    echo "使用方法:"
    echo "  bash mad/run_eval_mad_5cultural.sh MODEL_TYPE DATA_ID [MAX_SAMPLES] [RANDOM_P]"
    echo ""
    echo "参数:"
...
OUTPUT_DIR="/root/autodl-fs/mad_5cultural_results/${MODEL_NAME}_${DATA_NAME}_${TIMESTAMP}"
mkdir -p $OUTPUT_DIR
...
echo "========================================" | tee $LOG_FILE
echo "5-Agent Cultural MAD Evaluation" | tee -a $LOG_FILE
echo "========================================" | tee -a $LOG_FILE
...
echo "Cultural Agents: Asia, Western, South America, Oceania, Africa" | tee -a $LOG_FILE
...
echo "Starting 5-Agent Cultural MAD evaluation..." | tee -a $LOG_FILE
echo "" | tee -a $LOG_FILE

# 获取脚本所在目录（支持从项目根目录或任意目录调用）
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# 构建命令
CMD="python $SCRIPT_DIR/eval_mad_5cultural.py \
...
if [ -f "$OUTPUT_DIR/summary_statistics.json" ]; then
    ...
    python -c "import json; data=json.load(open('$OUTPUT_DIR/summary_statistics.json')); print(f\"  Final: {data['accuracy']:.2%}\"); print(f\"  Consensus Rate: {data['consensus_rate']:.2%}\"); print(f\"  MAD Gain: {data['mad_gain']:+.2%}\")" | tee -a $LOG_FILE
fi
