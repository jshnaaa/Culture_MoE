...
TRAIN_FILE="/root/autodl-fs/your_train_data.json"
OUTPUT_DIR="/root/autodl-fs/output/classification_lora_$(date +%Y%m%d_%H%M%S)"

# ✅ 使用 torchrun 启动分布式训练（LLaMA LoRA 微调）
# ⚠️ 重要：使用 LoRA 时，freeze_llama 必须设为 False
torchrun \
    --nproc_per_node=2 \
    --master_port=29500 \
