...
if [ $? -eq 0 ]; then
    ...
    echo "  ├── generated_answers.json (生成的答案样本)"
    ...
    echo ""
    echo "  # View generated answers (first 10):"
    echo "  cat $OUTPUT_DIR/generated_answers.json | python -c \"import json,sys; data=json.load(sys.stdin); [print(f'Q: {item[\\\"input\\\"][:100]}...\\nA: {item[\\\"generated_answer\\\"]}\\nTrue: {item[\\\"true_answer\\\"]}\\n---') for item in data[:10]]\""
    echo ""
    echo "  # Check accuracy:"
